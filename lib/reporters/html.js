...
internals.Reporter.prototype.end = function (notebook) {
    ...
    context.coverage.cov.files = Hoek.flatten(context.coverage.cov.files.map((file) => {
        ...
        const sourcemap = SourceMapSupport.retrieveSourceMap(file.filename);
        let smc = new SourceMapConsumer(sourcemap.map);
        if (!smc.hasContentsOfAllSources() && Fs.existsSync(file.filename)) {
            const generator = SourceMapGenerator.fromSourceMap(smc);
            generator.setSourceContent(sourcemap.url, Fs.readFileSync(file.filename, 'utf8'));
            smc = SourceMapConsumer(generator.toJSON());
        }
